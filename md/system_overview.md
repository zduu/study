...
* **核心功能**：根据四个关键变量重新生成循环系统参数
* **主要功能**：
  * 接收四个关键变量：透平入口温度、压比、ORC膨胀比等
  * 参数计算：调用`generate_cycle_parameters`函数计算所有系统参数
  * 生成新参数文件：将计算的参数保存为新的JSON文件
* **命令行接口**：支持通过命令行参数直接设置关键变量并生成配置
* **依赖关系**：与`generate_cycle_parameters.py`功能类似，但提供更简洁的命令行接口
...
#### `plot_pr_sensitivity.py` 和 `plot_pr_sensitivity_cn.py`
* **核心功能**：绘制参数敏感性分析的结果图表
* **主要功能**：
...
* **图表设计**：
  * 采用简洁的曲线展示，不包含最高点标注
  * 使用不同颜色区分各性能指标
  * 支持中英文两种图表标签(分别由`plot_pr_sensitivity.py`和`plot_pr_sensitivity_cn.py`生成)
...
#### `pr_sensitivity_plot.png` 和 `pr_sensitivity_plot_cn.png`
* **功能**：参数敏感性分析的可视化图表
* **内容**：展示压比与系统性能指标的关系曲线
* **用途**：直观展示系统性能趋势
* **设计特点**：简洁明了的图表设计，不包含最高点标注，便于用户自行分析

#### `pr_exergy_efficiency_plot.png` 和 `pr_exergy_efficiency_plot_cn.png`
* **功能**：压比对火用效率的影响分析图
* **内容**：展示压比与总火用效率的关系，并标注卡诺效率
* **用途**：分析系统在火用效率上的表现及与理论极限的对比
* **设计特点**：简洁明了的图表设计，不包含最高点标注
