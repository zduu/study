...
if __name__ == '__main__':
    # 设置输出重定向到文件
    tee = TeeOutput('cycle_components_output.txt')
    sys.stdout = tee
...
1. 代码整体功能
---------------
本代码实现了SCBC/ORC联合循环系统中各个组件的数学模型。主要包括:
- 压缩机模型(MC, RC)
- 透平模型(T, TO)
- 泵模型(PO)
- 换热器模型(HTR, LTR, ER, GO, CS, CO)

2. 核心组件模型说明
----------------
2.1 压缩机模型 (model_compressor_MC)
- 功能: 模拟SCBC主压缩机(MC)和再压缩机(RC)
- 输入参数:
  * state_in: 进口状态点
  * P_out_Pa: 出口压力
  * eta_isen: 等熵效率
- 计算过程:
  ...
  * 考虑效率损失
  * 计算实际出口状态
- 与论文对应: 对应论文中压缩机效率ηC=0.85的设定

2.2 透平模型 (model_turbine_T)
- 功能: 模拟SCBC透平(T)和ORC透平(TO)
- 输入参数:
  * state_in: 进口状态点
  * P_out_Pa: 出口压力
  * eta_isen: 等熵效率
- 计算过程:
  ...
  * 考虑效率损失
  * 计算实际出口状态
- 与论文对应: 对应论文中透平效率ηT=0.9的设定

2.3 泵模型 (model_pump_ORC)
- 功能: 模拟ORC循环中的泵(PO)
- 输入参数:
  * state_in: 进口状态点
  * P_out_Pa: 出口压力
...
- 计算过程:
  ...
  * 考虑效率损失
  * 计算实际出口状态
- 与论文对应: 对应论文中泵效率ηP=0.75的设定

2.4 换热器模型 (model_heat_exchanger_effectiveness)
- 功能: 模拟各类换热器(HTR, LTR)
- 输入参数:
  * state_hot_in: 热流体进口状态
  * state_cold_in: 冷流体进口状态
  * effectiveness: 换热器效能
  * hot_fluid_is_C_min_side: 热流体是否为最小热容流体
- 计算过程:
  * 计算最大可能换热量
  * 考虑效能影响
  * 计算实际换热量和出口状态
- 与论文对应: 对应论文中HTR和LTR效能η=0.86的设定

2.5 蒸发器模型 (model_evaporator_GO)
- 功能: 模拟SCBC/ORC耦合蒸发器(GO)
- 输入参数:
  * state_hot_in: SCBC侧进口状态
  * state_cold_in: ORC侧进口状态
  * 出口条件(温度或焓)
- 计算过程:
  * 计算换热量
  * 确定出口状态
  * 确保能量平衡
- 与论文对应: 对应论文中GO换热器的设计参数

2.6 冷却器模型 (model_cooler_set_T_out)
- 功能: 模拟各类冷却器(CS, CO)
- 输入参数:
  * state_in: 进口状态
  * T_out_K: 目标出口温度
  * target_state_is_saturated_liquid: 是否要求出口为饱和液体
- 计算过程:
  * 计算冷却过程
  * 确定出口状态
  * 计算换热量
...
2.7 加热器模型 (model_heater_set_T_out)
- 功能: 模拟吸热器(ER)
- 输入参数:
  * state_in: 进口状态
  * T_out_K: 目标出口温度
