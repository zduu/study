import json
import contextlib
from init.state_point_calculator import StatePoint, to_kelvin, to_pascal
...
1. 代码整体功能
---------------
本代码实现了SCBC/ORC联合循环系统的完整模拟。主要包括:
- 参数加载与配置
- SCBC循环模拟
- ORC循环模拟
- 联合循环性能计算
...
2. 核心函数说明
-------------
2.1 参数加载函数 (load_cycle_parameters)
- 功能: 从JSON文件加载系统参数
- 输入参数:
  * filename: 参数文件名
- 返回: 参数字典
...
2.2 SCBC循环模拟函数 (simulate_scbc_orc_cycle)
- 功能: 模拟SCBC循环运行
- 输入参数:
  * params: 系统参数字典
- 计算过程:
  ...
  * 执行高温侧迭代
  * 计算系统性能
- 与论文对应: 对应论文中SCBC循环的分析过程

2.3 ORC循环模拟函数 (simulate_orc_standalone)
- 功能: 模拟ORC循环运行
- 输入参数:
  * orc_params: ORC循环参数
  * common_params: 通用参数
  * intermediate_scbc_data: SCBC中间数据
- 计算过程:
  * 初始化状态点
  * 计算泵和透平
  * 处理蒸发器耦合
  * 计算ORC性能
- 与论文对应: 对应论文中ORC循环的分析过程
